...
import { ticketService, TicketCreateData, EventEdition, Ticket } from "@/services/ticket.service";
...
export default function EditTicketPage() {
  ...
  const fetchTicket = async () => {
    try {
      ...
      // const response = await tenantApi.getTicketById(ticketId, token || undefined);
      ...
      setFormData({
        ...
        ticket_start_date: formatDate(ticket.ticket_start_date),
        ticket_end_date: formatDate(ticket.ticket_end_date),
        status: ticket.status,
        sort_order: ticket.sort_order,
        is_nft_enabled: ticket.is_nft_enabled,
        event_edition_ids: [],
      });
      
      // Set selected events from ticket data
      if (ticket.event_editions) {
        setSelectedEvents(ticket.event_editions.map(e => e.id));
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Method 1: Using ticketService (recommended - handles response structure)
      await ticketService.updateTicket(ticketId, dataToSubmit);
      
      // Method 2: Direct API call (alternative)
      // await tenantApi.updateTicket(ticketId, dataToSubmit, token || undefined);
      router.push("/admin/tickets");
    } catch (error: any) {
