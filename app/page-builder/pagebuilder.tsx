...
const moduleTypes = [
  { value: 'events', label: 'Events List', description: 'Display upcoming events' },
  { value: 'speakers', label: 'Speakers', description: 'Show event speakers' },
  { value: 'tickets', label: 'Tickets', description: 'Ticket purchasing options' },
...
export default function PageBuilder() {
  ...
  const getDefaultModuleContent = (type: string) => {
    switch (type) {
      ...
      case 'image':
        return { src: '/placeholder.svg?height=300&width=600', alt: 'Image', caption: '' }
      case 'form':
        return { fields: [] }
      default:
        return {}
                  ...
                  {moduleTypes.map((moduleType) => (
                    ...
                    >
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">{moduleType.description}</div>
                      </div>
                    </Button>
                  ))}
                </div>
                ...
                <div className="space-y-4">
                  {page.modules.length === 0 ? (
                    <div className="text-center py-12 text-muted-foreground">
                      <p>No modules added yet. Use the sidebar to add content modules.</p>
                    </div>
                  ) : (
...
function ModuleEditor({ module, onUpdate }: { module: PageModule; onUpdate: (updates: Partial<PageModule>) => void }) {
  switch (module.type) {
    case 'text':
      return (
        <div className="space-y-2">
          ...
          <Textarea
            ...
            onChange={(e) => onUpdate({ content: { ...module.content, text: e.target.value } })}
      ...
      return (
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) => onUpdate({ content: { ...module.content, src: e.target.value } })}
            placeholder="Image URL"
          />
          <Label>Alt Text</Label>
          <Input
            value={module.content.alt || ''}
            onChange={(e) => onUpdate({ content: { ...module.content, alt: e.target.value } })}
            placeholder="Alt text"
          />
          <Label>Caption</Label>
          <Input
            value={module.content.caption || ''}
            onChange={(e) => onUpdate({ content: { ...module.content, caption: e.target.value } })}
    ...
    case 'events':
      return (
        <div className="p-4 bg-gray-50 rounded">
          <p className="text-sm text-muted-foreground">This module will display your events list. Configure events in the Events section.</p>
        </div>
      )
    ...
    case 'speakers':
      return (
        <div className="p-4 bg-gray-50 rounded">
          <p className="text-sm text-muted-foreground">This module will display your speakers. Configure speakers in the Speakers section.</p>
        </div>
      )
    ...
    case 'tickets':
      return (
        <div className="p-4 bg-gray-50 rounded">
          <p className="text-sm text-muted-foreground">This module will display ticket options. Configure tickets in the Tickets section.</p>
        </div>
      )
    ...
    default:
      return (
        <div className="p-4 bg-gray-50 rounded">
          <p className="text-sm text-muted-foreground">Module configuration will appear here.</p>
        </div>
      )
  }
}
